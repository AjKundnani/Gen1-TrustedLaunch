...
6    |    Install Bootloader in re-partitioned boot disk:<ul><li>**For Ubuntu**: `grub-install /dev/sda`<li>**For RHEL & SLES** `sudo grub2-install /dev/sda`</ul>    |    ![grub execute](./artifacts/grubinstall.png)
...
11    |    **Non-Azure** Copy existing files in /boot/efi to temporary /mnt/folder.<ol><li>`sudo mount -t vfat /dev/disk/by-partlabel/EFI-system /mnt`<li>`sudo mv  /boot/efi/* /mnt`<li>`sudo umount /mnt`</li></ol><br/>*Note*: You can skil this step if there're any error related to file does not exists.    |    
...
14    |    **Non-Azure** Install the GRUB EFI bootloader.<br/>**Ubuntu/Debian:**<br/>`sudo grub-install --target=x86_64-efi /dev/sda`<br/>**RHEL:**<br/>`grub2-install --target=x86_64-efi /dev/sda`    |    ![grub2 efi install](./artifacts/07a-grub2-efi-install.png)<br/>![grub 2 efi install contd](./artifacts/07b-grub2-efi-install.png)
