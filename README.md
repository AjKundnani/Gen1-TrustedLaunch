...
12    |    **Non-Azure**<ol><li>Open `/etc/fstab` using command `sudo vi /etc/fstab`<li>Add the ESP mountpoint to /etc/fstab. (replace spaces with tab key)<br/>`/dev/disk/by-partlabel/EFI-system /boot/efi vfat defaults 0 2`<li>Save `/etc/fstab` using command in vi editor `wq`.    |    ![ESP Mount](./artifacts/06-ESP-Mount.png)
