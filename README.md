...
Operating System    |    Operating system should be [Trusted launch supported.](https://aka.ms/TrustedLaunch) except <ul><li>Windows Server 2016</li></ul><br/>**NOTE**:<ul><li>For Linux VMs, execute MBR to GPT locally on VM. Refer to steps [Linux MBR to GPT conversion](#linux-os-mbr-to-gpt-conversion)<li> Windows Server 2016 does not natively supports `MBR to GPT` conversion and requires offline upgrade process. For more details refer to `link pending`</li></ul>
...
7    |    **Non-Azure** Execute gdisk to add an `EFI System` partition (ESP) with partition type **ef00**. Recommended size is **+200M** <br/>**Command**: `gdisk /dev/sda`<ul><li>Command: **n**<li>Partition Number: `default`<li>First Sector:`default`<li>Last Sector:**+200M**<li>Partition type: **ef00**<li>Command: **w** to write the changes</li></ul> |    ![EF00 partition](./artifacts/03.On-PremiseEF02.png)
8    |    **Non-Azure** Execute gdisk to rename above created partition to `EFI-system`<br/>**Command**: `gdisk /dev/sda`<ul><li>Command: **c**<li>Partition Number: `From Step 7 above`<li>Enter Name:`EFI-system` (case-sensitive)<li>Command: **w** to write the changes</li></ul>    |    ![EFI-system rename](./artifacts/04-On-PremiseEFI-System.png)
